...
export class CommunicationSignalingClient implements SignalingClient {
      ...
      skypeTokenProvider: async (forceRefresh: boolean) => {
        if (forceRefresh) {
          ...
          if (this.tokenFetchRetries > MAX_NUMBER_OF_TOKEN_FETCH_RETRIES) {
            this.stop(true);
            throw new Error("Access token is expired and failed to fetch a valid one after " + MAX_NUMBER_OF_TOKEN_FETCH_RETRIES + " retries");
          }
        } else {
          this.tokenFetchRetries = 0;
        }

        return Promise.resolve((await credential.getToken()).token)
      },
      telemetryConfig: {
  ...
  public stop(isTokenExpired?: boolean): void {
