...
const toEventPayload = (event: ChatEventId, genericPayload: any): any => {
  ...
  if (event === "chatThreadCreated") {
    ...
    const chatParticipants: ChatParticipant[] = membersPayload.map(
      m => { return {
  ...
  if (event === "participantsAdded") {
    ...
    const participantsAddedPayload = JSON.parse(unescape(payload.participantsAdded)) as ChatParticipantPayload[];
    ...
    const chatParticipants: ChatParticipant[] = participantsAddedPayload.map(
      m => { return {
        id: constructIdentifierKindFromMri(m.participantId),
        displayName: m.displayName,
  ...
  if (event === "participantsRemoved") {
    ...
    const participantsRemovedPayload = JSON.parse(unescape(payload.participantsRemoved)) as ChatParticipantPayload[];
    ...
    const chatParticipants: ChatParticipant[] = participantsRemovedPayload.map(
      m => { return {
        id: constructIdentifierKindFromMri(m.participantId),
        displayName: m.displayName,
        shareHistoryTime: new Date(m.shareHistoryTime)
